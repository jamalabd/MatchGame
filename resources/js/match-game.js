...
MatchGame.generateCardValues = function () {
  var sequentialCardValues = [];
  for (var value = 1; value <= 8; value++) {
     sequentialCardValues.push(value);
     sequentialCardValues.push(value);
  };
   var cardValues = [];
   while (sequentialCardValues.length>0) {
      var randomIndex = Math.floor(Math.random() * sequentialCardValues.length);
      var randomValues = sequentialCardValues.splice(randomIndex, 1)[0];
      cardValues.push(randomValues);
   };
   return cardValues;
};
/*
...
MatchGame.renderCards = function(cardValues, $game) {
  var colors = ['hsl(25,85%,65%)','hsl(55,85%,65%)','hsl(90,85%,65%)','hsl(160,85%,65%)','hsl(220,85%,65%)','hsl(265,85%,65%)','hsl(310,85%,65%)','hsl(360,85%,65%)'];
    ...
    for (var valueIndex = 0; valueIndex < cardValues.length; valueIndex++){
      ...
      var data = {value:value,isFlipped:false,color:color};
